...
import {createProject, updateProject, deleteProject, getProject} from "../../actions/project";
...
describe("Project Reducer / Actions", () => {
  ...
  it("should dispatch a redux API call to get a project's details", async () => {
    store.dispatch(getProject("someTestId"));
    await waitFor(() => expect(store.getActions()).toHaveLength(2));
    expect(store.getActions()[0].type).toBe("PROJECT_REQUEST_START");
